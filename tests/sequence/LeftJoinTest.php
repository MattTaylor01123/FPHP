...
class LeftJoinTest extends TestCase
{
    public function testArray()
    {
        ...
        $out2 = F::leftJoin(fn($v1, $v2) => $v1->userId === $v2->userId, fn(...$args) => F::mergeAllLeft(...$args), $seq1, $seq2);
        $this->assertEquals([
    ...
    public function testTraversable()
    {
        ...
        $out1 = F::leftJoin(fn($v1, $v2) => $v1->userId === $v2->userId, fn(...$args) => F::mergeAllLeft(...$args), $seq1, $seq2);
    ...
    public function testLaziness()
    {
        ...
        $out1 = F::leftJoin($fnPred, fn(...$args) => F::mergeAllLeft(...$args), $seq1, $seq2);
        ...
        $this->assertEquals((object)["userId" => 2, "first" => "Sarah", "last" => "Stone"], $v2);
       
        $iter->next();
        $this->assertTrue($iter->valid());
        $v3 = $iter->current();
        $this->assertEquals(8, $called);
        $this->assertEquals((object)["userId" => 3, "first" => "Jeremy", "last" => "Bates", "accountId" => 12, "email" => "jeremy.bates@hotmail.com"], $v3);
